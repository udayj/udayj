...
- **QuoteAssist** — multi-service agent system integrating Whatsapp, Telegram, Tally ERP, structured LLM tools calls that turns quotation requests received as text, voice notes, or photos into priced PDF quotations.
...
- **[VisionGrep](https://github.com/udayj/visiongrep)** — searches local image folders using a text description or reference image, with all inference running locally.
...
QuoteWatch & VisionGrep are public. The other systems are mostly private; the repositories below include complete applications, smaller tools, and components from related work.
...
- [visiongrep](https://github.com/udayj/visiongrep) — a Rust CLI for image search using CLIP and ONNX Runtime, with cached embeddings and structured output for scripts and AI agents.
